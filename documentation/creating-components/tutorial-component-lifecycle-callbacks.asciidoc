...
----
* Using methods in the `attachEvent` and `detachEvent` to get the UI or session is more convenient than using the `getUI()` method in Component, because these methods return values directly. The `getUI()` method in Component returns an `Optional<UI>`, because a component is not always attached. 
