...
As the starting point for the application, we use a Vaadin Application Starter, called Project Base for Vaadin platform. Project Bases are project stubs that have dependencies set up, and may include some example code. The steps of creating a Vaadin Application are as follows:

. Start by pointing your browser at https://vaadin.com/start[Vaadin Start]

. Select the Project Base for Vaadin platform by clicking it
