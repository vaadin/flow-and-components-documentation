...
Vaadin components are Polymer web components, which can be used both in JavaScript and through the server-side Java API.
+
See https://www.webcomponents.org/[webcomponents.org] for more information about Web Components.

Polymer::
Vaadin components are made with Polymer, an open-source JavaScript library for creating web components.
It simplifies how custom HTML elements are created, provides ways for data binding between the client and the server, and so forth.
+
See the tutorial https://vaadin.com/learn/training/v10-custom-elements[Creating Web Components with Polymer].
...
Now in Java, you can inject the text field and the label by their IDs as follows:
