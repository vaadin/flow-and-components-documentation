...
Or when creating Java UI Component API on the server side can make difficult for JavaScript developers, so Vaadin allows creating TypeScript views on client-side. In this case, the application can be started without creating a connected server-side UI instance,
which means applications can be stateless.
...
* A TypeScript view on the client side that using link:https://lit-element.polymer-project.org/[LitElement^] as a helper library.
...
** Gives JavaScript developers full access back-end from TypeScript by using Vaadin endpoint.
...
or access the Java class DTO and generate a corresponding TypeScript module from the client side. It is not necessary to understand how the client-to-server communication or Web Components work. This leaves you free to focus on creating components that work at a higher-abstraction level.
