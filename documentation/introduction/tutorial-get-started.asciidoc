...
click on the "com.vaadin.starter.starter.skeleton" package and choose menu:New[Class].
...
To finish off the setCustomer setter method, let's add setCustomer(null) to the end of the constructor to make sure that the save and delete buttons are disabled by default.
...
system like CDI events to completely decouple the components. We'll leave that out of this tutorial for
simplicity.
