...
Vaadin supports the Kotlin programming language through the Karibu-DSL library, which contains various extensions to use Vaadin with Kotlin.
...
----
@Route("")
class MyView : VerticalLayout() { <1>
    init {
        formLayout { <2>
            textField("Name:") { <3>
                description = "Last name, first name" <4>
            }
            textField("Age:") {
                width = "5em"
            }
...
----
<1> A view should always extend a Vaadin layout component.
<2> A Vaadin `FormLayout` component.
<3> A Vaadin `TextField` component with caption given.
<4> Define component attributes; `description` defines the tooltip text.
...
* http://www.vaadinonkotlin.eu/dsl_explained-v10.html[DSL: explained] – a more detailed description of building Vaadin UIs with the Vaadin-on-Kotlin DSL.
...
image::images/kotlin-starter.png[]
