...
=== Forward
Using forwardTo will <<Rerouting,reroute>> navigation and update the browser URL.
...
Calling `forwardTo` for the event will stop propagation of the event to other listeners not yet called.
Instead, a new navigation phase will be triggered based on the new navigation target, and new lifecycle events will be fired for the new forward navigation target instead.
