...
while `path/to/123/qwe` do not match the template.
...
It is the developer's responsibility to define these static routes in a way that they are independent of the order of annotation discovery, since the annotation discovery is not very well defined by the `AnnotatedElement`'s javadoc.
In case of dynamically registered routes the registration order is straightforward.
