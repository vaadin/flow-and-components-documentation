...
Route template parameters are defined as segments within `@Route`, `@RouteAlias` and `@RoutePrefix` annotation values, using `:parameter_name[modifier][(regex)]` syntax, where:
...
* `regex` is also optional and defines the regex used to match the parameter value.
