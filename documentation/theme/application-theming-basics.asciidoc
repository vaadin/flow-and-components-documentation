...
you need to put the CSS content in a `.css` file, and then instruct flow to import it by
using the Java `@CssImport` annotation.
...
[NOTE]
In addition to `@CssImport`, you still can use the `@StyleSheet` classic annotation for those
