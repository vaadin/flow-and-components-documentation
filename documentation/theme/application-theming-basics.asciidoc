...
[NOTE]
As seen on the <<using-component-themes#,8.5. _Using Component Themes_>> tutorial, the Lumo theme is used by
default if it is available in the classpath and no other `@Theme` is defined.

The contents of the `shared-styles.css` are discussed further in <<theming-crash-course#,8.3. _Theming Web Components_>>.
...
There is an example of this in the <<theming-crash-course#,8.3. _Theming Web Components_>> chapter too.
