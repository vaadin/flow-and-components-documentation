...
In Vaadin Platform there are two ready-made component themes: _Lumo_ (which is the main theme for all Vaadin components) and _Material._
...
If a different theme or no theme is enabled, no actual changes for the component's look and feel are made even if theme variants are set.

To reiterate an earlier point: by default, if no explicit theme configuration is done and `vaadin-lumo-theme`
dependency is present in the classpath, Lumo theme is used.
