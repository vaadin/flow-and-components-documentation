...
You can customize the way error messages display, by configuring the `Label` for each binding. The label is used to show the status of the field. The label can be used for validation errors, as well as, confirmation and helper messages.

*Example*: Configuring `Label` for email and minimum length validation. 
...
As an alternative to using labels, you can set a custom validation status handler, using the `withValidationStatusHandler` method. This allows you to customize how the binder displays error messages and is more flexible than `AbstractComponent.setComponentError(com.vaadin.server.ErrorMessage)`.
...
*Example*: Using the `convertToModel` and `convertToPresentation` methods.
