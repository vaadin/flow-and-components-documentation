...
* The second callback finds out how many items there are in total in the datasource matching with current filters. Since there is no filtering empoyed in this example, the returned value is equal to the total amount of items in the datasource.
