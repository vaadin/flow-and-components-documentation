...
<2> The item count callback is optional.
Use it if getting the count is cheap and/or a real scroll bar size is desired.

You may want to filter the items in combo box by some value type other than a string.
If your backend is able to fetch the items by this value, you can set the filter converter along with fetch callback.

In the example below, the combo box uses a custom filter converter to filter
the persons by a given age.
