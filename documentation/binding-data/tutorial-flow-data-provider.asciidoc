...
*The `ComboBox` examples listed below use an API available only in Vaadin 18+.*

For example, you can handle filterable lazy data binding to a Spring Data
...
----
<1> Here we use only the fetch callback for lazy loading with undefined item count. The item count callback is optional.
<2> Add `%` marks to filter for an SQL "LIKE" query.
Use it if getting the count is cheap and/or a real scroll bar size is desired.

You may want to filter the items in `ComboBox` by some value type other than a string.
If your backend is able to fetch the items by this value, you can set the filter converter along with fetch callback.

In the example below, the `ComboBox` uses a custom filter converter to filter the persons by a given age:
