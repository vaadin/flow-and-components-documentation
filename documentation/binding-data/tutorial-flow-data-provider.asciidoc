...
All list components have a method called `setItems` to define what items are to be displayed and they have an interface called `DataProvider` for more finegrained control of the displayed data.

== Changing how items are displayed

Most components by default use `toString()` method to display items. If that don't suite for you, you can change the behaviour by configuring the component. Components are configured with one or more callbacks that define how to display the items.
...
== Showing in memory data

The easiest way to pass data to listing components is to use `setItems` method. It accepts a collection, an array or a stream of items.
