...
* The second callback finds out how many items are currently available in the datasource matching the first callback. Since there is no filtering empoyed in this example, the returned value is equal to the total amount of items in the datasource.
...
----
* The results of the first and second callbacks must be symmetric. The second callback must always return the correct number of items fullfilling the first query callback. Since this example doesn't impose any restrictions such as filtering, the returned number of items is equal to the total items in the datasource.
...
----
* Note, `getCount` method takes into account the filtering criteria when returning amount of avaiable items.
