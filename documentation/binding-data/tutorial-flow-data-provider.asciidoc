...
* Optionally, allows item editing directly in the component.
...
All list components share a common mechanism - a `DataProvider` - that fetches the items to display.

== Displaying List Items

. The first step in displaying list items is to define how the data displays.  
+
Items are generally loaded directly from memory or lazily from some kind of backend. Regardless of the location, all components are configured with one or more callbacks that define how to display the items. 
+
*Example*: A `ComboBox` component that lists status items and and uses the `Status.getLabel()` method to represent each status. There is also a `Grid` with two columns, Name and Year of birth. .
+
[source, java]
----
...
----
* It is not necessary to define an item caption provider if `Status.toString()` is implemented to return suitable text. `ComboBox` is configured to use `toString()` to find a caption to display, by default. 
+
. The next step is to provide data to display in the component. 
+
The easiest way to do this is to directly pass the values to show to `setItems`.
+
...
=== Creating a List Data Provider

As an alternative to assigning the items in the collection directly, you can create a `ListDataProvider` that contains the items a component should use. 
...
*Example*: `PersonService` interface with an update method that returns a new instance of the item. _(Other functionality omitted. )_
...
*Example*: Data provider to update a persons name and save it to the backend.
