...
Vaadin acts as a bridge between Java backend services and a TypeScript
...
* `${endpoint}` — the url common part that every exposed service has.
...
[NOTE]
====
Service name and the method name are not case sensitive for Vaadin, so
