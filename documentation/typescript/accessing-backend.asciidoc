...
Basically, modules are files that have the following syntax:

- Allows using top-level `import` and `export` statements.

- The `export` statement makes the module's definitions available for the other modules.

- Could be imported by other modules using the `import` statement.
...
[source,typescript]
----
// define an interface and export as a module
