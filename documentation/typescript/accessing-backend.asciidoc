...
[WARNING]
Vaadin endpoint depends on Spring Boot auto-configuration. It will not work properly if the auto-configuration is disabled, for example, when you use `@EnableWebMvc`. A simple workaround is to remove the `@EnableWebMvc`, as stated in this link:https://docs.spring.io/spring-boot/docs/2.3.4.RELEASE/reference/html/spring-boot-features.html#boot-features-spring-mvc-auto-configuration[Spring Boot doc].
