...
In order to write the app bootstrapping code, or create views in TypeScript make sure that:
 ...
 - and the `index.html` page template and the `index.ts` entry point do exist in the `frontend/` folder. If they don't, follow the <<upgrading-from-vaadin14#step-2,instruction>> to create them.
