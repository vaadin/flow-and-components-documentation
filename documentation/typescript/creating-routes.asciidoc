...
When using client-side routing it is possible to compose the page layout without any server intervention, so as the initial application page also known as <<application-shell#application-shell,Application Shell>> can be shown when in offline mode.
