...
One more thing you need to pay attention in Vaadin 15+ is the annotations on the `MainLayout/MainView` class should be moved to a class implements AppShellConfigurator, refer <<application-shell#java-annotations, set annotations for modifying Bootstrap Page>> for more detail.