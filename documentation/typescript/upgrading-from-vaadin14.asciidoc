...
One more thing to pay attention to in Vaadin 15+ is that annotations in the `MainLayout / MainView` class must be moved to a class that implements the `AppShellConfigurator` interface, see <<application-shell#java-annotations, set annotations to modify Bootstrap page>> for more details.