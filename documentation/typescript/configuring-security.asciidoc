...
if not specified in Java code explicitly, before invoking a endpoint method, the presence of `Principal` object in the the request is required.
...
== Accessing user information in a endpoint method

When access to the `UserPrincipal` is required in a endpoint, call the `VaadinRequest.getCurrent()` method to access the `HttpServletRequest` API.
