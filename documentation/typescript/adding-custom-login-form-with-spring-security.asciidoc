...
a `loggedIn` boolean stored in the https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage[localStorage] to check if the user is logged in. The boolean needs to be reset when logging out. Use localStorage allows navigating to sub views without always having to check authentication from the backend. 
