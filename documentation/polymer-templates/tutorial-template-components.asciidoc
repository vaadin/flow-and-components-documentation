...
Here is a template file with a placeholder `div` element with a `"content"` identifier. This element is mapped to a `Div` component in Java code below so that we can add a `Component` as a child for it.
...
wired to the template DOM element and the 'container' field in the Java class.
