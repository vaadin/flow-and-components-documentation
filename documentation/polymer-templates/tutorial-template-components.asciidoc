...
for an element only defined in the PolymerTemplate JavaScript file for the client side to
use on the server.
...
Here is the content of the JavaScript module creating the template with a placeholder `div` element with a `"content"` identifier. This element is mapped to a `Div` component in Java code below to set a `Component` as a child for it.
