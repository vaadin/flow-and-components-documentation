...
It is recommended to use LitElement template language for your components, but PolymerTemplate (see <<tutorial-polymer-template-components-in-slot#,PolymerTemplate, Dynamically Adding Server-side Components to Templates>>) is also supported.
...
*Example*: `<slot>` element with default content in a TypeScript client-side template.
...
*Example* Using the `name` attribute in the `<slot>` element in a TypeScript client-side template.
...
*Example*: Nested slot elements in a TypeScript client-side template.
