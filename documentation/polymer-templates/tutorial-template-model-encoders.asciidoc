...
want this identifier of a bean to be able to reference it)? In this case you can use `@Encode`
...
By default the `path` value is `""` which means that an encoder is applied to the property itself.

You can use any type to encode to any supported type. Another usecase for encoders may be
