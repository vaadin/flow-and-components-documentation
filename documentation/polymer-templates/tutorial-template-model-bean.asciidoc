...
* If you later update the `Person person` bean created in the constructor, the model remains unaffected. The bean values are copied by `setPerson` and the bean is not attached to the model in any way.
