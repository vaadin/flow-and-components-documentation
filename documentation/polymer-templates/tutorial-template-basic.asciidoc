...
The demo shows three ways of creating as simple view: using Components, using pure Element API and the template approach that we'll show here.
...
The server side class has a method annotated with `@EventHandler`. The method is called from the client side, triggered
