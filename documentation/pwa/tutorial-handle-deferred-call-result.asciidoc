...
When deferring an endpoint call, its result is only available after the actual submission over network.
...
However, if the closes the application offline and later reopens online, there could be deferred calls in the queue, which are not sent yet.
