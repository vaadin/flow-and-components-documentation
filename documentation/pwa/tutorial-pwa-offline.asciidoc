...
Adding the <<tutorial-pwa-pwa-with-flow#,`@PWA` annotation>> on your application shell class enables the service worker, which automatically serves the client-side views offline.
