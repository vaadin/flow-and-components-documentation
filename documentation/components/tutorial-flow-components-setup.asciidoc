...
instead of some particular version.

This means that build results may differ from time to time because of the new libraries' versions released.
Vaadin provides a bom that fixes all vaadin-related dependencies, so that the separate components can be added the following way:
