...
`Grid` is normally bound to a data provider. See <<../binding-data/tutorial-flow-data-provider#,Data Providers>> for more.
...
[source, java]
----
// in single-selection mode, only one item is selected
grid.select(defaultItem);

// switch to multi-selection mode, clears selection
grid.setSelectionMode(SelectionMode.MULTI);
// Select items 2-4
...
selectionModel.addMultiSelectionListener(event -> {
    ...
    // Allow deleting only if there's a selection
...
grid.addColumn(new ComponentRenderer<>(person -> {
    ...
    // button for saving the name to the backend
