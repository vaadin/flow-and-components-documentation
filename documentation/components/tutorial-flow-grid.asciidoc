...
All property-based columns are sortable by default if the type of property of the column implements `Comparable`. Most of the usual data types, such as Strings, Numbers, primitive types and Date/LocalDate/LocalDateTime are Comparable, and thus they are sortable by default.
