...
* When using a custom web-component or a Vaadin element in a template renderer, remember to import the component. This can be done using link:https://vaadin.com/docs/v14/flow/polymer-templates/tutorial-template-subtemplate.html[`@JsModule`]  or link:https://vaadin.com/api/platform/com/vaadin/flow/component/dependency/Uses.html[`@Uses`], if the component has a server-side counterpart. This will ensure all StyleSheet, HtmlImport, JavaScript dependencies for component are loaded when the Grid is used.
...
To learn how to customize the styles inside a component’s shadow DOM, see <<../../themes/styling-components#,Styling Components>>.
