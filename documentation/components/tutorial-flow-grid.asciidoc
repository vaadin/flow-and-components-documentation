...
See the Polymer 3 documentation (https://polymer-library.polymer-project.org/3.0/api/elements/dom-repeat) for more.
...
See <<../binding-data/tutorial-flow-data-provider#,5.5. _Showing a List of Data with Data Providers_>> for more.
...
* *Backend sorting* is applied by providing a list of `QuerySortOrder` objects to your `DataProvider`, that typically passes the sort hints to the backend code, and in some cases all the way to database queries. See <<../binding-data/tutorial-flow-data-provider#Sorting,5.5. _Showing a List of Data with Data Providers_>> for more.
