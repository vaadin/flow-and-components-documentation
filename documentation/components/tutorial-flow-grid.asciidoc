...
The `Grid` doesn't implement the `HasValue` interface directly. Other selection components and many input fields typically do implement this interface. For this reason, selection handling in the `Grid` is different from typical selection components. The `Grid` supports three selection options: single selection, multiple selection, and no selection. Each option is defined by a specific selection model.
...
Using sort property, you can override or customise the property or multiple properties that are used for sorting the column. This option includes both in-memory and backend sorting. Properties is defined at the time of column construction and uses a sort property name.
