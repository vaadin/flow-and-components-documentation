...
|devmode.transpile |
To improve startup time, Vaadin 14.2 no longer transpiles frontend resources to ES5 in development mode.
ES5 transpilation is required for the application to function properly on older browsers which are not ES6-compliant (primarily IE11).
If you want to test your application in development mode on these browsers, set this property to `true` to enable transpilation.
Note that transpilation is always enabled in production mode, regardless of this property.
*This feature is available only in 14.2 and newer versions of the Vaadin Platform 14 release series.*