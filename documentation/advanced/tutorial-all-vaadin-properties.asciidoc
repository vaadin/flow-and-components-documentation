...
|devmode.polymer.version |
Allows to specify Polymer version in development mode. By default the polymer version is `3.2.0`. *This feature is available only in Vaadin Platform 14.1 and newer.*

|disable.pnpm |
When set to `true`, disables `pnpm` which means `npm` will be used to handle frontend resources. *This feature is available only in Vaadin Platform 14.1 and newer.*
