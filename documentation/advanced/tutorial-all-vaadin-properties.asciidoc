...
Allows to specify Polymer version in development mode. By default the polymer version is `3.2.0`.

|disable.pnpm |
When set to `true`, disables `pnpm` which means `npm` will be used to handle frontend resources.
