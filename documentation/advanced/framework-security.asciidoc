...
Vaadin does not automatically set the `X-Frame-Options` HTTP header, because many times apps should run inside frames. The developer can set this header either in the server options or by using the Java Servlet API (adding the header to `VaadinResponse.getCurrent()`).
