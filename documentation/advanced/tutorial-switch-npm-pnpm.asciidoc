...
Starting from Vaadin 14.2 _pnpm_ (AKA _performant npm_) tool is available for managing frontend dependencies as a faster alternative to _npm_, which is still the default tool. The benefit of pnpm is that it uses a shared repository, which allows reducing the build time.

If pnpm is not installed globally the framework will install it automatically.
...
Alternatively, the property can be also set to the `vaadin-maven-plugin`, using `pnpmEnable`. Note that you should add it for both `prepare-frontend` and `build-frontend` goals.
