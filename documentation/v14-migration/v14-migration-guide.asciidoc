...
npm instead of pnpm, you may disable pnpm (see <<../advanced/tutorial-all-vaadin-properties#,All Vaadin properties>>
...
The invocation of pnpm/npm is fully automated by Vaadin, the only
thing you need to make sure is that _Node.js_ (version 10 or later) and npm
(version 5.6 or later) are installed and available on the system path when running in development node or when making a production build.* If they are not installed, the application
