...
in development mode. This requires transpilation of the frontend resources to IE11 compatible
ES5 syntax, and is not enabled by default for performance reasons since Vaadin 14.2.
Transpilation can be enabled by setting the system property `vaadin.devmode.transpile` to `true`.
