...
run alongside as a development server. In production mode, webpack creates
(transpiled and minified) bundles from the resources inside the `frontend` folder.
As long as your project file structure is as specified in this guide, these steps
...
will also release a migration tool that helps you convert your Vaadin 14 compatibility mode
applications to Vaadin 14 running the new toolset.
