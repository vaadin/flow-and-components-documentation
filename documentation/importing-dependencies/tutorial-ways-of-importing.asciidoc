...
As discussed in <<../theme/including-style-sheets#,Including style sheets>> and <<tutorial-importing#,Importing HTML and JavaScript>>, you can import client-side dependencies or resource files using:
...
The `custom-css.js` uses the technique for wrapping CSS into JavaScript presented in <<../theme/including-style-sheets#,Including Style Sheets>>.
