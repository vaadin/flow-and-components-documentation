...
If you need to change the name (or location) of the static assets war (`vaadin-portlet-static`), you can do that via `portlet.static.resources.mapping` system property.
For example, if you would like to serve static resources from `[portal-url]/resources/vaadin-static-resources/`, you will need to +

- Rename static war to `vaadin-static-resources` in the pom.xml
- Build the project with `-Dportlet.static.resources.mapping="/resources/vaadin-static-resources/"
- Place the created `vaadin-static-resources.war` under `/resources` on your web server
