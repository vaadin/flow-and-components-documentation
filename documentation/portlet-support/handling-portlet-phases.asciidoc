...
When a Vaadin portlet is rendered the first time a new view instance is created, and each portlet instance on a portal page has its own instance for each browser window in which the page is open.
