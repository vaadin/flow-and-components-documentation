...
- You have a Vaadin Portlet module (either built from scratch or based on `addressbook-portlet` or `base-starter-flow-portlet`); and
...
----
Note that the portlet modules should be added before the static asset module (here `myproject-bundle`).
...
=== Remove Static Assets War Build Configuration From Portlet (If Applicable)
...
Since the target project builds its own static asset module, these are no longer needed in the portlet module itself.
Next, we need to check that the portlet module itself is configured properly.

=== Ensure the New Portlet Module is Configured Properly

In the `pom.xml` of the new portlet module, enable `<attachClasses>`:
[source,xml]
----
<plugin>
...
Note that this is a server URL (not a local file path).
...
Note that the `<warName>` of the static asset bundle must be exactly `vaadin-portlet-static`.
