...
Quick steps for the impatient. If everything does not work as smoothly as expected or you want to understand how things tie together, read on.

1. Go to https://www.webjars.org/ and deploy the webcomponent version as a "Bower GitHub" webjar
2. Go to https://vaadin.com/start/v10-component, create a project for the webcomponent to integrate and write Java APIs
3. Assemble the finalized component for Vaadin Directory using `mvn install -Pdirectory`

[NOTE]
It is assumed that the web component can be found from Bower.
...
[NOTE]
The webjar is immediately deployed to https://dl.bintray.com/webjars/maven/org/webjars/bowergithub/ and later synchronized to Maven central. Use `dl.bintray.com` in your `pom.xml` to be able to use it immediately. 
...
Now you are set to create the Java API, for more details see <<<dummy/../creating-java-api-for-a-web-component>>>
