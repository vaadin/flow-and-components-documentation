...
`paper-slider` is an input type component where the user can decide what the value is.
...
The value should be synchronized automatically from the client to the server, when the user changes it, and from the server to the client, when changing it from code.
...
This is only a workaround. But since there is no other API to control this behaviour, you can use it.
...
It makes more sense if you call `increment()` directly from client side or some other business logic.
...
See the method's javadoc for more information about supported types.
