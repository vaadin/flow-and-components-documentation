...
The component class, e.g. `PaperSlider.java` you get when using the component starter, see <<<dummy/../integrating-a-web-componnents>>> is just a stub which handles the imports. There are multiple ways to interact with a web component but the typical pattern is:
...
```
The setter will now set the given property to the requested value and the setter will return the property value, or `false` as the default if the property has not been set (this should match the default of the web component property).
