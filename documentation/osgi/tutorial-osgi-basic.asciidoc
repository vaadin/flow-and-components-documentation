...
https://github.com/vaadin/flow/issues/4367[Classloader bug]. Once it's fixed there will be no need for it.
...
But there is no dedicated `webapp` folder for OSGi bundles.
You should instead register your resource via the way provided by Vaadin OSGi
...
Therefore, if you have a bundle which contains routes or other classes whose 
...
Please note that there are many transitive dependencies which also need to be deployed.
...
should also be deployed of course) but the latter bundle requires `ServiceLoader` 
...
The result the folder will be packaged in the jar archive and the resources
