...
[NOTE]
When you have more than one bundle created by Vaadin, note that only one of them
should have ‍‍‍`VaadinServlet` registration. It's also a good practice to have a
core bundle where you have this `FixedVaadinServlet` and make sure that this
bundle is active all the time by making all other bundles dependent on it.
