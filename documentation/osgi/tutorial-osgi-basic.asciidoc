...
and may very because of transitive dependencies.
Here is some of required external dependencies (the versions are omitted):
...
Other dependencies requires some OSGi features which needs to be deployed at 
...
Also Vaadin OSGi support uses OSGi Compendium API (which allows register OSGi
service using declarative services annotations). And you have to deploy 
(if your OSGI container doesn't have it out of the box) implementation 
bundle to support this API. 

[NOTE]
We have OSGi base starter project which is ready to use and it declares
...
As a result all required bundles becomes deployed to the OSGi container.
See https://github.com/vaadin/base-starter-flow-osgi.

In your project you will want most likely to use some ready-made Vaadin components like Vaadin Button.
