...
| CustomLayout | HMTL or PolymerTemplate | See notes below.
...
Read the <<../components/tutorial-enabled-state.asciidoc#,Component Enabled State>> tutorial for more details.
...
This was has its roots in a time when the differences between browsers were big,
