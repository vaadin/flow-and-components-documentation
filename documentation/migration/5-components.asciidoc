...
| AbsoluteLayout | - | Not planned. Similar functionality in V10 can be achieved using DIV elements and CSS.
...
| CheckBoxGroup | - | Not in V10, take `vaadin-radio-button-flow` 1.1. Included in Vaadin 12 (12/2018). Also available as an add-on in Directory for V10
...
| ComboBox | ComboBox | https://vaadin.com/components/vaadin-combo-box/java-examples[Demo]. Note that the V10 ComboBox is not lazy-loading between server and client, that is planned for Vaadin 12.

| ContextMenu (official add-on) | ContextMenu (V12) | Not in V10, take `vaadin-context-menu` 1.0 which is V10 compatible. Included in Vaadin 12 (12/2018)
...
| DateTimeField | - | Planned for Q4 2018. `<input type="datetime">` is supported in some browsers.
...
| NativeSelect | - | Planned, by new component `DropdownMenu`. Timeline is Q3 2018. As a temporary workaround, low level element API can be used with `<select>` element.
...
| RichTextArea | - | Planned for Vaadin 12 (12/2018). There are Web Components available, check https://vaadin.com/directory[vaadin.com/directory]
