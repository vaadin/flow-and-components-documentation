...
The `ViewDisplay` concept is replaced in Vaadin 10+ with the `RouterLayout` interface, but now it is possible to have nested hierarchies,
...
10+ supports optional parameters. It is not anymore possible to use the fragment style (`#!`) navigation state from V8
