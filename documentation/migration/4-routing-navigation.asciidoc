...
in Vaadin 10+ the routes are configured declaratively on each navigation target and are the same for the entire application:
...
Thus, when migrating from using `Navigator`, any `View` from Vaadin 8 can be migrated to Vaadin 10+ by removing the registration
from the now optional `UI` class and instead applying the `@Route` annotation the class.
*Note* that there is no `View` interface in Vaadin 10+, but instead the class must be a `Component`!
...
The `ViewDisplay` concept has been replaced in Vaadin 10+ with the `RouterLayout` interface, but now it is possible to have nested hierarchies,
by using the `@ParentLayout` annotation to set one `RouterLayout` class as the parent of another.

There is no `ViewProvider` in Vaadin 10+ as it is not needed.
...
to get full understanding on how to structure your Vaadin 10+ application.
