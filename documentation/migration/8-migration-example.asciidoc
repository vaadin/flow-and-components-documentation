...
* Setting theme name is done by calling
`getElement().getThemeList().add(String)` instead of
`addStyleName(String)`.
...
* The equivalent of `addStyleName` that adds a theme to a component is
`getElement().getThemeList()::add`. A nicer API for this is coming to
Vaadin 12.
...
profile named productionMode is added to pom.xml of ui module.
