...
This document explains how to deploy a Vaadin Flow application on Jetty.

Jetty is an open-source project providing an HTTP server, HTTP client, and javax.servlet container.

Jetty servers can be deployed in 2 different ways:

. Embedded Jetty
    * (Jetty Maven Plugin, only during development)
    * Programmatically
. Standalone Jetty
    * WAR
    * Exploded directory
...
Using Jetty has the advantage that it can be instantiated and used in a standard Java program, meaning it does not need to be separately installed on the server.
...
This programmatic configuration also requires some extra dependencies in `pom.xml`:
...
Deploying to a standalone Jetty server can be done either using a WAR file or an exploded directory with the application in it.
...
It is actually an extracted WAR file (since WAR files are ZIP files).
