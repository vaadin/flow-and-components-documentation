...
[NOTE]
In the case of creating a production build, it is important to activate the maven profile `production`.
Production mode can be activated in the following way: `mvn jetty -PproductionMode`.
