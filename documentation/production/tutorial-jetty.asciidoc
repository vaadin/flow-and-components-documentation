...
[NOTE]
It might require additional dependencies, such as: `jetty-annotations`,`jetty-continuation`,`javax-websocket-server-impl`, ...
...
[NOTE]
In the case of creating a production build, it is important to
Remember to activate the production mode when the application is ready for its deployment.
Production Mode can be activated in the following way: `mvn jetty:run-exploded -PproductionMode`.
