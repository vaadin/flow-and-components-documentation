...
In addition to standard CDI contexts, the Vaadin CDI (https://vaadin.com/directory/component/vaadin-cdi) add-on introduces new contexts.

Vaadin CDI contexts are conceptually similar to <<../spring/tutorial-spring-scopes#,9.4. _Vaadin Spring Scopes_>>.
...
The Vaadin CDI (https://vaadin.com/directory/component/vaadin-cdi) add-on introduces the `@VaadinServiceScoped`, `@VaadinSessionScoped`, `@NormalUIScoped`, `@NormalRouteScoped` normal scopes..
...
See Asynchronous Updates (<<../advanced/tutorial-push-access#asynchronous-updates,>>) for more about using push.

=== `@VaadinServiceScoped` Context

The `@VaadinServiceScoped` context manages the beans during the Vaadin service lifecycle. The lifecycle of the service is the same as the lifecycle of its Vaadin servlet. See Vaadin Servlet and Service (<<../advanced/tutorial-application-lifecycle#vaadin-servlet-and-service,>>) for more about the Vaadin service.

For beans that are automatically picked up by `VaadinService`, you need to use the `@VaadinServiceEnabled` annotation, together with the `@VaadinServiceScoped` annotation. See <<tutorial-cdi-service-beans#,10.5. _Vaadin Service Interfaces as CDI Beans_>> for more.
...
See User Session (<<../advanced/tutorial-application-lifecycle#application.lifecycle.session,>>) for more.
...
See Loading a UI (<<../advanced/tutorial-application-lifecycle#application.lifecycle.ui,>>) for more about the `UI` lifecycle.
...
See <<../routing/tutorial-routing-annotation#,6.1. _Using the @Route Annotation_>> and <<../routing/tutorial-router-layout#,6.3. _Router Layouts and Nested Router Targets_>> for more about route targets, route layouts, and the route chain.
