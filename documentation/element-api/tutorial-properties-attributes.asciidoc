...
* You cannot set an element's `style` attribute using the `element.setAttribute` method. 
* You can use the `element.getAttribute('style')` method to get all style rules in a single concatenated string.
