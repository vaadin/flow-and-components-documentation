...
in <<tutorial-binder#,Form binder>> and <<tutorial-binder-load#,Form binder>> tutorials respectively.
...
=== Binding data to Text an Number fields

No extra action is needed for configuring the vaadin components present in the `text-field` package.
...
There are two elements in Vaadin to deal with booleans: `vaadin-checkbox` and `vaadin-radio-buttom`,
both work fine with form binders, but none of them have validation and failure styling, thus you need some
...
Options these components can be set by calling a server service that provides the string list.
...
For single selection `vaadin-combo-box`, `vaadin-radio-group`, `vaadin-list-box` should be used.
All of them can be selected by providing the selected item value as a string.
